...
export async function GET(req: NextRequest) {
  ...
  const tempC = searchParams.get("tempC") ? Number(searchParams.get("tempC")) : undefined;
  ...
  if (OPENTRIPMAP_API_KEY) {
    promises.push(fetchOpenTripMap({ lat, lon, radius, wantIndoor, wantOutdoor }));
  }
  ...
  // Return top 6
  return NextResponse.json({ suggestions: pick(results, 6) }, { headers: { "Cache-Control": "s-maxage=300, stale-while-revalidate=600" } });
}
...
async function fetchOpenTripMap(params: {
  ...
  const indoorKinds = ["museums", "theatres_and_entertainments", "galleries", "cultural"]; // indoor-ish
  const outdoorKinds = ["parks", "gardens", "natural", "view_points", "urban_environment"]; // outdoor-ish
  const kinds = wantIndoor ? indoorKinds : wantOutdoor ? outdoorKinds : indoorKinds.concat(outdoorKinds);
  ...
  const suggestions: Suggestion[] = data
    ...
    .map((p) => ({
      ...
      indoor: kinds.some((k) => indoorKinds.includes(k)) ? true : kinds.some((k) => outdoorKinds.includes(k)) ? false : null,
...
async function fetchTicketmaster(params: { lat: number; lon: number; night: boolean }): Promise<Suggestion[]> {
  const { lat, lon, night } = params;
  ...
  const events = data?._embedded?.events as any[] | undefined;
  if (!Array.isArray(events)) return [];
  const suggestions: Suggestion[] = events.slice(0, 12).map((e) => {
